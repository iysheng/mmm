...
class c_myUI : public c_wnd
{
	virtual void on_init_children(void) {
		...
		// // 配置 data_rate 和 refresh_rate
		// // 这个 data_rate 和 refresh rate 什么作用呢？
		// // 前面的是采样速率， 256 点/秒
		// // 后面的是刷新速度 17 ms/帧，即每秒大概 58 帧
		...
		c_wave_multi_ctrl *p_wave_multi;
		p_wave_multi = (c_wave_multi_ctrl*)get_wnd_ptr(ID_WAVE3);
		p_wave_multi->set_back_color(GL_RGB(0XF7,0X8A,0X3B));
		p_wave_multi->set_wave_speed(1);
		p_wave_multi->set_wave_in_out_rate(128, 17);
		p_wave_multi->set_wave_name_font((const LATTICE_FONT_INFO*)c_theme::get_font(FONT_DEFAULT));

		p_wave_multi->set_wave(&m_wave_buffer1, 0);
		// p_wave_multi->set_wave_name(gs_name_wave[0]);
		p_wave_multi->set_max_min(170, 110, 0);
		p_wave_multi->set_wave_color(GL_RGB(0, 255, 0), 0);
		p_wave_multi->set_wave_name_color(GL_RGB(0, 255, 0), 0);

		p_wave_multi->set_wave(&m_wave_buffer2, 1);
		// p_wave_multi->set_wave_name(gs_name_wave[1], 1);
		p_wave_multi->set_max_min(100, 0, 1);
		p_wave_multi->set_wave_color(GL_RGB(41, 189, 189), 1);
		p_wave_multi->set_wave_name_color(GL_RGB(41, 189, 189), 1);

		p_wave_multi->set_wave(&m_wave_buffer3, 2);
		// p_wave_multi->set_wave_name(gs_name_wave[2], 2);
		p_wave_multi->set_max_min(180, 80, 2);
		p_wave_multi->set_wave_color(GL_RGB(255, 255, 0), 2);
		p_wave_multi->set_wave_name_color(GL_RGB(255, 255, 0), 2);
	}
...
static c_wave_multi_ctrl s_wave3;
...
void create_ui(void* phy_fb, int screen_width, int screen_height, int color_bytes, struct DISPLAY_DRIVER* driver)
{
	...
	while(1)
	{
		...
		s_myUI.m_wave_buffer1.write_wave_data(s_wave_data1[data_index3 % sizeof(s_wave_data1)]);
		s_myUI.m_wave_buffer2.write_wave_data(s_wave_data2[data_index3 % sizeof(s_wave_data2)]);
		s_myUI.m_wave_buffer3.write_wave_data(s_wave_data3[data_index3++ % sizeof(s_wave_data3)]);
		// s_myUI.m_wave_buffer3.write_wave_data(s_wave_data3[data_index3++ % sizeof(s_wave_data3)]);
...
#endif
		s_wave3.refresh_wave(wave_index, 0);
		s_wave3.refresh_wave(wave_index, 1);
		s_wave3.refresh_wave(wave_index, 2, true);
		wave_index++;
		thread_sleep(17);
	}
}
...
extern "C" \
void startHelloWave(void* phy_fb, int width, int height, int color_bytes)
{
    create_ui(phy_fb, width, height, color_bytes, NULL);
}

void* getUiOfHelloWave(int* width, int* height, bool force_update)
{
    (void)force_update;
